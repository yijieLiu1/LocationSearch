...
public class FilterHandler implements HttpHandler {
    ...
    public FilterHandler(DataGenerator dataGenerator) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        if ("GET".equals(exchange.getRequestMethod())) {
            try {
                Log.logger.info("接收到有无筛选请求");

                // 构造json格式数据
                ObjectMapper mapper = new ObjectMapper();
                String response = mapper.writeValueAsString(dataGenerator.getRectangleFilterFlagStr());

                exchange.sendResponseHeaders(200, response.length());
                OutputStream outputStream = exchange.getResponseBody();
                outputStream.write(response.getBytes());
                outputStream.close();
                Log.logger.info("成功返回有无筛选");
            } catch (Exception ex) {
                // 构造响应
